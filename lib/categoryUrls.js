...
// Appended to every supercollections.in link sent to a customer, so an order placed after
// clicking through from WhatsApp shows up in WooCommerce's own traffic-source reporting.
const WHATSAPP_UTM_QUERY = 'utm_source=whatsapp&utm_medium=bot&utm_campaign=supercollections';

export function withUtm(url) {
    return `${url}${url.includes('?') ? '&' : '?'}${WHATSAPP_UTM_QUERY}`;
}
...
export function getCategoryUrl(categoryName) {
    ...
    return withUtm(FALLBACK_SHOP_URL);
}
