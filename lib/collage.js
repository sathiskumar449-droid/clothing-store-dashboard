...
const CHARACTER_PATHS = {
    '#': 'M 4,2 L 4,22 M 10,2 L 10,22 M 1,8 L 13,8 M 1,16 L 13,16',
    '0': 'M 2,2 H 12 V 22 H 2 Z',
    '1': 'M 7,2 V 22 M 3,6 L 7,2',
    '2': 'M 2,2 H 12 V 12 H 2 V 22 H 12',
    '3': 'M 2,2 H 12 V 22 H 2 M 2,12 H 12',
    '4': 'M 2,2 V 12 H 12 M 12,2 V 22',
    '5': 'M 12,2 H 2 V 12 H 12 V 22 H 2',
    '6': 'M 12,2 H 2 V 22 H 12 V 12 H 2',
    '7': 'M 2,2 H 12 V 22',
    '8': 'M 2,2 H 12 V 22 H 2 Z M 2,12 H 12',
    '9': 'M 2,22 H 12 V 2 H 2 V 12 H 12'
};

function getNumberSvgPaths(numberString) {
    const chars = String(numberString).split('');
    const charWidth = 14;
    const charSpacing = 4;
    const boxHeight = 24;
    
    const totalWidth = chars.length * charWidth + (chars.length - 1) * charSpacing;
    const startX = (85 - totalWidth) / 2 + 10;
    const startY = (50 - boxHeight) / 2 + 10;
    
    let pathsSvg = '';
    chars.forEach((char, idx) => {
        const pathD = CHARACTER_PATHS[char];
        if (pathD) {
            const x = startX + idx * (charWidth + charSpacing);
            const y = startY;
            pathsSvg += `<path d="${pathD}" transform="translate(${x}, ${y})" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" />\n`;
        }
    });
    return pathsSvg;
}
...
async function createPlaceholder(number) {
    const svgText = `
      <svg width="300" height="300">
        ...
        <text x="150" y="150" font-family="Arial, Segoe UI, sans-serif" font-size="20" fill="#888888" text-anchor="middle">No Image</text>
        <rect x="10" y="10" width="85" height="50" rx="8" fill="rgba(0, 0, 0, 0.75)" />
        ${getNumberSvgPaths(`#${number}`)}
      </svg>
    `;
