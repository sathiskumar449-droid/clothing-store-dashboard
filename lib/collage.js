...
async function createPlaceholder(number) {
    ...
    const svgText = `
      <svg width="300" height="300">
        ...
        <text x="150" y="160" font-family="Arial, sans-serif" font-size="22" font-weight="600" fill="#adb5bd" text-anchor="middle">No Image</text>
        <rect x="15" y="15" width="60" height="60" rx="12" fill="#000000" />
        <text x="45" y="45" font-family="Arial, sans-serif" font-size="${fontSize}" font-weight="900" fill="#ffffff" text-anchor="middle" dominant-baseline="central">${number}</text>
      </svg>
    `;
...
async function generateCollage(productBuffers) {
    ...
    const footerSvg = `
      <svg width="${width}" height="${footerHeight}">
        ...
        <text x="${width / 2}" y="${footerHeight / 2}" font-family="Arial, sans-serif" font-size="16" font-weight="bold" fill="#111111" text-anchor="middle" dominant-baseline="central">Reply with the product number to continue.</text>
      </svg>
    `;
