...
export async function createProductCollage(productsPage, startNumber, allProducts = [], allowPlaceholder = false) {
    try {
        // 1. Process all product images in parallel
        //    If placeholders are allowed, keep the grid layout even when a remote image fetch fails.
        const processingPromises = productsPage.map((p, idx) => {
            ...
            return processProductImage(imgUri, startNumber + idx, allowPlaceholder, false);
        });
